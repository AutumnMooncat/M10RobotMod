...
public class SelfDestructAction extends AbstractGameAction {
    ...
    public SelfDestructAction(AbstractPlayer player, AbstractMonster monster, int[] multiDamage, DamageInfo.DamageType damageType, int hpLoss, boolean doDamageFalloff) {
        this.p = player;
        this.m = monster;
        this.multiDamage = multiDamage;
        this.damageType = damageType;
        this.hpLoss = hpLoss;
        this.doFalloff = doDamageFalloff;
        xBb = p.drawX;
        yBd = p.drawY;
        xBh = p.hb.cX;
        yBh = p.hb.cY;
    ...
    public void update() {
        ...
        if (waitTimer > 0) {
            ...
            if (!hits.isEmpty()) {
                ...
                int damage = multiDamage[AbstractDungeon.getMonsters().monsters.indexOf(aM)];
                if(doFalloff) {
                    damage -= (int) Math.ceil(new Vector2(aM.hb.cX - p.hb.cX, aM.hb.cY - p.hb.cY).len() /damageFalloffIncrements) - 1;
                }
                if (damage > 0) {
                    AbstractDungeon.effectList.add(new FlashAtkImgEffect(aM.hb.cX, aM.hb.cY, AbstractGameAction.AttackEffect.FIRE, true));
                    aM.damage(new DamageInfo(p, damage, damageType));
                }
                hits.remove(0);
